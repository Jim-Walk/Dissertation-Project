...
I found that using generics in Rust made reading error messages difficult, but easier to parse once the offending code was removed into a smaller example, and stripped of its generic type. Generics in Rust necessitate verbose syntax, for example, \texttt{T::from(0).unwrap()} is used to generate a zero of type T. 
