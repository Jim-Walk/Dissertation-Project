...
\end{minted}
\captionof{listing}{Rustc: borrow checker in action}
...
\end{minted}
\captionof{listing}{Rust: Babel Stream add, before applying idiomatic style}
    \label{lst:iters-b}
\end{code}
\end{minipage}\hfill
...
\end{minted}
\captionof{listing}{Rust: Babel Stream add, after applying idiomatic style}
\label{lst:iters-a}
\end{code}
\end{minipage}
...
A bug in my sparse matrix vector multiplication implementation was found at this stage. When launched with certain parameters, the C version ran without error. The Rust version would panic and fail every time, whilst initilising the \texttt{col\_index} with the error message:
...
\end{minted}
\captionof{listing}{Rust: Needless range loop}
...
for (k, <item>) in old_cluster_centres.iter()
                                       ...
                                       .skip(1) {
\end{minted}
\captionof{listing}{Rust: Clippy's suggested iterator}
\end{code}
...
\end{minted}
\captionof{listing}{Rust: Babel Stream add, parallelised}
\label{lst:iters-p}
\end{code}
...
sum1
\end{minted}
\captionof{listing}{Rust: Serial dot product}
\label{lst:dot-for}
\end{code}
...
\end{minted}
\captionof{listing}{Rust: Parallel dot product}
\label{lst:dot-fold}
\end{code}
...
\end{minted}
\captionof{listing}{Rust: Serial SpMV}
\end{code}
\end{minipage}\hfill
\begin{minipage}{.49\textwidth}
\begin{code}
...
\end{minted}
\label{lst:spmv-par}
\captionof{listing}{Rust: Parallel SpMV}
\end{code}
\end{minipage}
...
\end{minted}
\captionof{listing}{Rust: K-means serial E-step}
...
\end{minted}
\captionof{listing}{Rust: K-means parallel E-step}
\label{lst:kmeanspar}
\end{code}
\end{minipage}
...
\end{minted}
\captionof{listing}{C: Babel Stream parallel initialisation}
...
\end{minted}
\captionof{listing}{Rust: Babel Stream parallel initialisation}
\label{lst:parInit}
\end{code}
\end{minipage}
...
I will use the peak memory bandwidth recorded by the Babel Stream kernel in my Roofline Model. For the peak Flop/s, I will use the theoretical figure calculated from the processor's specification. I am making this choice because the maximum calculated Flop/s is difficult to find due to a processor's vector units.
